use crate::{ast::token::literal::Literal, error::{AlthreadError, AlthreadResult, ErrorType}};

use super::{channels::Channels, instruction::{Instruction, InstructionType, ProgramCode}, str_to_expr_error, GlobalAction, GlobalActions, GlobalMemory, Memory};
...
impl<'a> RunningProgramState<'a> {
    ...
    pub fn next_global(&mut self, globals: &mut GlobalMemory, channels: &mut Channels, next_pid: &mut usize, global_state_id: u64) -> AlthreadResult<(GlobalActions, Vec<Instruction>)> {
    ...
    pub fn is_next_instruction_global(&mut self) -> bool {
        self.current_instruction().map_or(true, |inst| !inst.control.is_local())
    }

    fn next(&mut self, globals: &mut GlobalMemory, channels: &mut Channels, next_pid: &mut usize, global_state_id: u64) -> AlthreadResult<Option<GlobalAction>> {
