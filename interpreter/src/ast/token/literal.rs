...
impl Literal {
    ...
    pub fn add(&self, other: &Self) -> Result<Self, String> {
        match (self, other) {
            ...
            (Self::String(s), other) => Ok(Self::String(format!("{}{}", s, other.to_string()))),
            (other, Self::String(s)) => Ok(Self::String(format!("{}{}", other.to_string(), s))),
            (i, j) => Err(format!(
                "Cannot add {} and {}",
                i.get_datatype(),
                j.get_datatype()
    ...
    pub fn shift_left(&self, other: &Self) -> Result<Self, String> {
        match (self, other) {
            (Self::Int(i), Self::Int(j)) if *j >= 0 && *j < i64::BITS as i64 => i
    ...
    pub fn bit_and(&self, other: &Self) -> Result<Self, String> {
        match (self, other) {
            (Self::Int(i), Self::Int(j)) => Ok(Self::Int(i & j)),
            (a, b) => Err(format!(
                "Cannot perform bitwise AND between {} and {}",
                a.get_datatype(),
                b.get_datatype()
            )),
        }
    ...
    pub fn bit_or(&self, other: &Self) -> Result<Self, String> {
        match (self, other) {
            (Self::Int(i), Self::Int(j)) => Ok(Self::Int(i | j)),
            (a, b) => Err(format!(
                "Cannot perform bitwise OR between {} and {}",
                a.get_datatype(),
                b.get_datatype()
            )),
        }
