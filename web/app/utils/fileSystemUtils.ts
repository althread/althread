...
export const getFilePathFromEntry = (
...
export const findEntryAndParent = (
	fs: FileSystemEntry[],
	path: string,
): {
	entry: FileSystemEntry;
	...
	for (let i = 0; i < parts.length - 1; i++) {
		const part = parts[i];
		const dirEntry = currentLevel.find(
			(e) => e.name === part && e.type === "directory",
		);
