...
import { loadFileContent, saveFileContent } from "./storage";
...
export class WebPackageManager {
	...
	public async parseAltToml(): Promise<AltTomlConfig | null> {
		...
		try {
			...
			const dependenciesResult =
				wasmModule.parse_dependencies_from_toml(content);
			...
			if (dependenciesResult instanceof Map) {
				...
				typeof dependenciesResult === "object" &&
			...
			const packageMatch = content.match(
				/\[package\]\s*name\s*=\s*"([^"]+)"\s*version\s*=\s*"([^"]+)"/s,
			);
			if (!packageMatch) {
	...
	public async addDependency(
		...
		try {
			...
			try {
				updatedContent = wasmModule.add_dependency_to_toml(
					content,
					packageName,
					version,
				);
	...
	public async installDependencies(): Promise<void> {
		...
		const installPromises = Object.entries(config.dependencies).map(
			([name, version]) => this.installPackage(name, version),
		);
	...
	public async installPackage(
		...
		if (this.packageCache.has(cacheKey)) {
			...
			await this.loadPackageIntoFileSystem(
				packageName,
				this.packageCache.get(cacheKey)!,
			);
			return;
		}
	...
	private async fetchRemotePackage(
		packageName: string,
		version: string,
		...
		try {
			...
			if (!user || !repo) {
				throw new Error(
					`Invalid GitHub package format - user: "${user}", repo: "${repo}"`,
				);
			}
			...
			if (Object.keys(files).length === 0) {
				console.warn(
					`No .alt files found in ${packageName}, creating minimal package`,
				);
			...
			console.log(
				`Fetched ${Object.keys(files).length} files from ${packageName}`,
			);
...
`,
	...
	private async fetchDirectoryContents(
		...
		try {
			...
			for (const item of contents) {
				if (
					item.type === "file" &&
					(item.name.endsWith(".alt") || item.name === "alt.toml")
				) {
					// Fetch file content
	...
	private async loadPackageIntoFileSystem(
		packageName: string,
		...
		let depsDir = this.fileSystem.find(
			(entry) => entry.name === "deps" && entry.type === "directory",
		);
		if (!depsDir) {
			depsDir = {
				id: crypto.randomUUID(),
		...
		let packageDir = depsDir.children?.find(
			(entry) => entry.name === packageDirName,
		);
		if (!packageDir) {
			packageDir = {
				id: crypto.randomUUID(),
		...
		for (const [filePath, content] of Object.entries(packageInfo.files)) {
			...
			for (let i = 0; i < pathParts.length - 1; i++) {
				...
				let subDir = currentDir.children?.find(
					(entry) => entry.name === dirName && entry.type === "directory",
				);
				if (!subDir) {
					subDir = {
						id: crypto.randomUUID(),
		...
		console.log(
			`✓ Package loaded: ${packageName} (${Object.keys(packageInfo.files).length} files)`,
		);
	}
	...
	public async removePackage(packageName: string): Promise<void> {
		// Remove from cache
		const keysToRemove = Array.from(this.packageCache.keys()).filter((key) =>
		...
		const depsDir = this.fileSystem.find(
			(entry) => entry.name === "deps" && entry.type === "directory",
		);
		if (depsDir && depsDir.children) {
			const packageDirName = packageName.replace(/[^a-zA-Z0-9]/g, "_");
			depsDir.children = depsDir.children.filter(
				(entry) => entry.name !== packageDirName,
			);
			this.setFileSystem([...this.fileSystem]);
		}
