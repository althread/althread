...
main {
	...
	validate: (code: string) => {
		...
		if (!hasAwaitFirst) {
			issues.push(
				"You must use 'await first { ... }' to wait for multiple conditions.",
			);
		}

		const hasReceivePort1 =
			/receive\s+port1\s*\(\s*\w+\s*\)\s*=>\s*\{\s*print\s*\(\s*"Received from port 1"\s*\)\s*;?\s*\}/.test(
				code,
			);
		const hasReceivePort2 =
			/receive\s+port2\s*\(\s*\w+\s*\)\s*=>\s*\{\s*print\s*\(\s*"Received from port 2"\s*\)\s*;?\s*\}/.test(
				code,
			);

		if (!hasReceivePort1) {
			issues.push(
				"You need to handle 'receive port1' and print 'Received from port 1'.",
			);
		}
		if (!hasReceivePort2) {
			issues.push(
				"You need to handle 'receive port2' and print 'Received from port 2'.",
			);
		}
		...
		if (issues.length === 0) {
			return {
				success: true,
				message: "Great! You've mastered multi-condition waiting.",
			};
		} else {
