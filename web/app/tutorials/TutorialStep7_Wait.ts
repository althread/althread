...
main {
	...
	validate: (code: string) => {
		const hasSharedReady =
			/shared\s*{[^}]*READY\s*:\s*bool\s*=\s*false;[^}]*}/s.test(code);
		const hasProgramSignal =
			/program\s+Signal\s*\(\s*\)\s*{[^}]*READY\s*=\s*true;[^}]*print\("Signal: READY set to true"\);[^}]*}/s.test(
				code,
			);
		const hasProgramListener =
			/program\s+Listener\s*\(\s*\)\s*{[^}]*print\("Listener: Waiting..."\);[^}]*await\s+READY(\s*==\s*true)?;[^}]*print\("Listener: Ready!"\);[^}]*}/s.test(
				code,
			);
		const runsBothInMain =
			/main\s*{[\s\S]*(run\s+Signal\s*\(\s*\)\s*;[\s\S]*run\s+Listener\s*\(\s*\)\s*;|run\s+Listener\s*\(\s*\)\s*;[\s\S]*run\s+Signal\s*\(\s*\)\s*;)[\s\S]*}/s.test(
				code,
			);
		...
		if (!hasProgramSignal)
			issues.push(
				"Program 'Signal' to set READY to true, and print 'Signal: READY set to true'",
			);
		if (!hasProgramListener)
			issues.push(
				"Program 'Listener' to print 'Listener: Waiting...', wait for READY (e.g. 'await READY;'), and print 'Listener: Ready!'",
			);
		if (!runsBothInMain)
			issues.push(
				"running both Signal and Listener programs in the main block",
			);
		return {
			success: false,
