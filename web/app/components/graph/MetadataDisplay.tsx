...
export default function MetadataDisplay({ nodes }: { nodes: VisNode[] }) {
	const calculateMetadata = () => {
		...
		const depthReached = nodes.reduce(
			(maxDepth, node) => Math.max(maxDepth, node.level ?? 0),
			0,
		);
	...
	return (
		<div class="metadata-display">
			...
			<div>
				...
				<pre> {JSON.stringify(metadata.statesStored, null, 2)}</pre>
			</div>
			<div>
				<span>Max depth reached:</span>
				<pre> {JSON.stringify(metadata.depthReached, null, 2)}</pre>
			</div>
			<div>
				<span>Violations:</span>
				<pre> {JSON.stringify(metadata.violations, null, 2)}</pre>
			</div>
		</div>
	);
}
