...
const Tutorial: Component = () => {
	...
	const findIndexFromParam = (param: string | undefined): number => {
		...
		const index = tutorialOrder.findIndex(
			(key) => tutorials[key]?.name.toLowerCase() === param.toLowerCase(),
		);
		return index !== -1 ? index : 0; // Default to first if param is invalid
	};

	const [code, setCode] = createSignal("");
	const [currentTutorialIndex, setCurrentTutorialIndex] = createSignal(
		findIndexFromParam(params.stepName),
	);
	const [activeTab, setActiveTab] = createSignal<"validation" | "result">(
		"validation",
	);
	const [validationOutput, setValidationOutput] = createSignal<{
		message: string;
		success: boolean;
	} | null>(null);
	...
	const getLocalStorageKey = (index: number): string => {
		if (index >= 0 && index < tutorialOrder.length) {
			...
			console.warn(
				`Tutorial 'name' property not found for key ${tutorialKey}, using key itself for localStorage.`,
			);
			return `althread_tutorial_${tutorialKey}`;
		}
		console.error(
			"Attempted to get localStorage key with out-of-bounds index:",
			index,
		);
	...
	createEffect(() => {
		...
		if (tutorialData) {
			const savedCode = localStorage.getItem(
				getLocalStorageKey(currentTutorialIndex()),
			);
			...
			setValidationOutput({
				success: false,
				message: `Tutorial data for '${tutorialKey}' is missing.`,
			});
		}
	});
	...
	const currentTutorial = (): TutorialStep | undefined =>
		tutorials[tutorialOrder[currentTutorialIndex()]];
	...
	const handleValidateCode = () => {
		...
		if (tutorial) {
			...
			setValidationOutput({
				success: false,
				message: "Error: Current tutorial not found for validation.",
			});
		}
	};
	...
	const handleRunCode = async () => {
		...
		try {
			...
			if (runtimeError) {
				...
				const formattedError = formatAlthreadError(
					runtimeError,
					currentEditorCode,
				).message;
				setExecutionResult(
			...
			<div class="tutorial-header">
				...
				<div class="tutorial-step-selector-wrapper">
					<select
						...
						onChange={(e) => {
							const selectedStepKey = (e.currentTarget as HTMLSelectElement)
								.value;
					...
					>
						<For each={tutorialOrder}>
							{(stepKey) => {
								const tutorial = tutorials[stepKey];
								return (
									<option value={stepKey}>
										{tutorial ? tutorial.displayName : `Loading: ${stepKey}`}
									</option>
								);
							}}
						</For>
			...
			<Resizable id="tutorial-content-area">
				...
				>
					<div
						...
						innerHTML={
							currentTutorial()?.content
								? renderTutorialContent(currentTutorial()!.content)
					...
					<div class="navigation-buttons">
						...
						<button
							...
							disabled={currentTutorialIndex() === tutorialOrder.length - 1}
						>
				...
				>
					<div class="editor-pane-area">
						<div class="editor-toolbar">
							...
							<div class="action-buttons-center editor-toolbar-actions">
								<button
									...
									class="vscode-button validate-button"
								>
						...
						<div
							ref={editorInstance.ref}
							class="editor-tutorial-instance-wrapper"
						></div>
					</div>
						...
						<div class="tab-content">
							<div class="console">
								{activeTab() === "validation" && (
									<div
										class={`validation-message ${validationOutput() ? (validationOutput()!.success ? "success" : "error") : ""}`}
									>
										{validationOutput()
								...
								{activeTab() === "result" && (
									<div
										class={`validation-message${executionError() ? " execution-error-box" : ""}`}
									>
										<pre>
											{executionResult()
												? executionResult()
