...
export default function PackageManagerDialog(props: PackageManagerDialogProps) {
	const [packageManager, setPackageManager] =
		createSignal<WebPackageManager | null>(null);
	const [altTomlConfig, setAltTomlConfig] = createSignal<AltTomlConfig | null>(
		null,
	);
	...
	const loadCurrentState = async () => {
		...
		try {
			...
			setError(
				err instanceof Error ? err.message : "Failed to load package state",
			);
		} finally {
			setLoading(false);
		}
	...
	return (
		<Show when={props.isOpen}>
			...
			>
				<div
					class="modal-content package-manager-dialog"
					role="dialog"
					aria-modal="true"
				>
					...
					<div class="modal-body">
						...
						<Show when={error()}>
							<div class="error-message">
								{error()}
								<button type="button" class="close-msg" onClick={clearMessages}>
									×
								</button>
							</div>
						</Show>
							...
							<div class="tab-content">
								<Show when={!altTomlConfig()}>
									<div class="init-project-section">
										...
										<div class="form-group">
											<label for="pm-project-version">Version:</label>
											<input
												id="pm-project-version"
												type="text"
								...
								<Show when={altTomlConfig()}>
									...
									<div class="add-dependency-section">
										...
										<div class="form-group">
											<label for="pm-package-name">Package Name:</label>
											<input
												id="pm-package-name"
												type="text"
												value={packageName()}
										...
										<div class="form-group">
											<label for="pm-package-version">Version:</label>
											<input
												id="pm-package-version"
												type="text"
									...
									<div class="dependencies-section">
										...
										<Show
											when={
												Object.keys(altTomlConfig()?.dependencies || {})
													.length > 0
											}
										>
												...
												<For each={cachedPackages()}>
													{(pkg) => (
														<div class="cached-package">
															...
															<span class="package-type" data-type={pkg.type}>
																{pkg.type}
															</span>
														</div>
													)}
