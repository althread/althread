...
export default function InteractivePanel(props: InteractivePanelProps) {
	...
	const [activeTab, setActiveTab] = createSignal<"choices" | "state">(
		"choices",
	);
	const [rightPanelTab, setRightPanelTab] = createSignal<
		"console" | "execution" | "msg_flow" | "vm_states"
	>("console");
	...
	const handleExecutionTabClick = (
		tab: "console" | "execution" | "msg_flow" | "vm_states",
	) => {
	...
	const formatInstructions = (state: NextStateOption): JSX.Element => {
		...
		const hasMessageOp = state.instructions.some(
			(inst) =>
				inst.includes("SEND") ||
				inst.includes("RECEIVE") ||
				inst.includes("DELIVER"),
		);
			...
			if (props.deadlockDetected) {
				return (
					<div class="interactive-empty-state">
						...
						<p>
							The execution is blocked. Check the console or Message flow for
			...
			<div class="interactive-choices-container">
				<div class="choices-header">
					...
					<span class="choices-count">
						{props.interactiveStates.length} option
						{props.interactiveStates.length !== 1 ? "s" : ""}
					</span>
				</div>
				<div class="interactive-choices-list">
					{props.interactiveStates.map(
						(state: NextStateOption, index: number) => (
							<button
								type="button"
	...
	const renderVMState = () => {
		...
		return (
			<div
				class="vm-state-container"
				style="height: 100%; display: flex; flex-direction: column;"
			>
				<VMStateInspector node={state} onClose={() => {}} />
				<div
					class="interactive-empty-state"
					style="flex: 1; justify-content: flex-start; padding-top: 20px;"
				>
					<p style="font-size: 11px; opacity: 0.7;">
						Interactive state view. Use the choices tab to progress.
					</p>
				</div>
			</div>
		);
	...
	const renderRightPanelContent = () => {
		...
		} else if (props.isRun !== false) {
			// When running (not in check mode)
			if (rightPanelTab() === "console") {
				return (
					<div class="console">
						<pre>
							{props.stdout ||
								"The console output will appear here.\n\nAs you execute steps, any print statements or program output will be shown here in real-time."}
						</pre>
					</div>
				);
				...
				return (
					<div class="console">
						{renderMessageFlowGraph(
							props.interactiveMessageFlow || [],
			...
			<div class={`interactive-panel ${isMinimized() ? "minimized" : ""}`}>
				<div class="panel-header">
					...
					<div class="panel-controls">
						...
						>
							<i
								class={`codicon ${isMinimized() ? "codicon-chevron-up" : "codicon-chevron-down"}`}
							></i>
							...
							>
								...
								<div class="panel-content">
									<div class="panel-body">
										<Show
											when={activeTab() === "choices"}
											fallback={renderVMState()}
										>
