...
type FileExplorerProps = {
	...
	setGlobalFileCreation?: (
		creation: { type: "file" | "folder"; parentPath: string } | null,
	) => void;
};
...
const FileEntry = (props: {
	...
	const handlePaste = () => {
		setShowContextMenu(false);
		const destPath =
			props.entry.type === "directory" ? currentPath : props.path;
	...
	const handleRenameCommit = (newName: string) => {
		...
		if (newName && newName !== props.entry.name) {
			...
			if (
				props.checkNameConflict &&
				props.checkNameConflict(parentPath, newName)
			) {
		...
		if (draggedData && props.entry.type === "directory") {
			try {
				...
				for (const sourcePath of draggedPaths) {
					...
					if (
						props.checkNameConflict &&
						props.checkNameConflict(currentPath, movingName)
					) {
			...
			>
				...
				<Show when={isOpen()}>
					<div class="directory-children">
						...
						<Show
							when={props.creating && props.creating.parentPath === currentPath}
						>
							<div class="file-entry-input-wrapper">
								<div class="file-entry-input">
									<i
										class={`codicon codicon-${props.creating!.type === "file" ? "file" : "folder"}`}
									></i>
									<input
										ref={(el) => {
											if (
												el &&
												props.creating &&
												props.creating.parentPath === currentPath
											) {
		...
		<div
			...
			classList={{
				active:
					...
					props.getFilePath(props.activeFile) === currentPath,
				selected: isSelected() && !isRenaming(),
				dragging: isDragging(),
				"read-only": isInDepsDirectory(currentPath),
			}}
...
const FileExplorer = (props: FileExplorerProps) => {
	...
	const [localCreating, setLocalCreating] = createSignal<{
		type: "file" | "folder";
		parentPath: string;
	} | null>(null);
	const creating = () =>
		props.globalFileCreation !== undefined
			? props.globalFileCreation
			: localCreating();
	const setCreating = props.setGlobalFileCreation || setLocalCreating;

	const [isDragOver, setIsDragOver] = createSignal(false);
	const [clipboard, setClipboard] = createSignal<string[] | null>(null);
	const [isContextMenuOpenFor, setContextMenuOpenFor] = createSignal<
		string | null
	>(null);

	// Global edit state management - only one edit operation allowed at a time
	const [currentlyRenaming, setCurrentlyRenaming] = createSignal<string | null>(
		null,
	);
	...
	const handlePaste = (destPath: string) => {
		...
		for (const sourcePath of sources) {
			...
			const hasExtension =
				nameParts.length > 1 && nameParts[nameParts.length - 1].length > 0;
	...
	const [createCommitInProgress, setCreateCommitInProgress] =
		createSignal(false);
	...
	const handleDrop = (e: DragEvent) => {
		...
		console.log(
			"Drop event on root explorer with data:",
			e.dataTransfer?.getData("text/plain"),
		);
		...
		if (draggedData) {
			try {
				...
				for (const sourcePath of draggedPaths) {
					...
					if (
						props.checkNameConflict &&
						props.checkNameConflict("", movingName)
					) {
						// Show confirmation dialog
						if (props.showConfirmDialog) {
				...
				<div class="file-explorer-actions">
					<button
						onClick={() => {
							...
							if (props.selectedFiles.length === 1) {
								const selectedPath = props.selectedFiles[0];
								const selectedEntry = findEntryByPath(
									props.files,
									selectedPath,
								);
								if (selectedEntry && selectedEntry.type === "directory") {
					...
					<button
						...
						onClick={async () => {
							...
							const fileSystemRaw = localStorage.getItem(
								"althread-file-system",
							);
							let fileSystem: any[] = fileSystemRaw
								? JSON.parse(fileSystemRaw)
								: [];
									...
									if (entry.type === "file") {
										const content =
											localStorage.getItem(
												"althread-file-content-" + fullPath,
											) || "";
			...
			>
				...
				<Show when={creating() && creating()!.parentPath === ""}>
					<div class="file-entry-input-wrapper">
						<div class="file-entry-input">
							<i
								class={`codicon codicon-${creating()!.type === "file" ? "file" : "folder"}`}
							></i>
							<input
								ref={(el) => {
