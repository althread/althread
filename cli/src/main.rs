...
mod resolver;
use args::{
    AddCommand, CheckCommand, CliArguments, Command, CompileCommand, InitCommand, InstallCommand,
    RandomSearchCommand, RemoveCommand, RunCommand, UpdateCommand,
};
use clap::Parser;
...
pub fn run_interactive(
    ...
    compiled_project: althread::compiler::CompiledProject,
) {
...
pub fn add_command(cli_args: &AddCommand) {
    ...
    println!(
        "Adding remote dependency: {} ({})",
        dep_info.name, dep_info.url
    );
    ...
    println!(
        ...
        dep_info.url,
        if cli_args.dev { "dev" } else { "runtime" }
    );
...
pub fn remove_command(cli_args: &RemoveCommand) {
    ...
    if !removed {
        eprintln!(
            "Error: Dependency '{}' not found in alt.toml.",
            cli_args.dependency
        );
        exit(1);
    }
...
pub fn install_command(cli_args: &InstallCommand) {
    ...
    if !alt_toml_path.exists() {
        eprintln!(
            "Error: No alt.toml found. Run 'althread init' or 'althread add <dependency>' first."
        );
        exit(1);
    }
    ...
    match resolver.resolve_and_install_dependencies(&package, cli_args.force) {
        Ok((resolved_deps, version_changes)) => {
            ...
            if !resolved_deps.is_empty() {
                ...
                println!(
                    "  Resolved {} total dependencies (including transitive)",
                    resolved_deps.len()
                );
...
fn update_single_dependency(package: &mut Package, dep_name: &str) -> Result<bool, Box<dyn Error>> {
    ...
    if needs_update {
        ...
        let new_spec = match current_spec {
            DependencySpec::Simple(_) => DependencySpec::Simple(latest_version),
            DependencySpec::Detailed {
                features, optional, ..
            } => DependencySpec::Detailed {
        ...
        if package.dependencies.contains_key(dep_name) {
            ...
            package
                .dev_dependencies
                .insert(dep_name.to_string(), new_spec);
        }
